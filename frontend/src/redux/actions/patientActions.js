...
const patientActions = {
   getCalendar: () => {
      return async (dispatch, getState) => {
         try {
            let calendar = await axios.get("https://nutrimed.herokuapp.com/api/calendar")
      ...
      return async () => {
         try {
            let res = await axios.put(
               "https://nutrimed.herokuapp.com/api/patient/" + id,
               { description },
               {
                  headers: {
      ...
      return async () => {
         try {
            let res = await axios.post(
               `https://nutrimed.herokuapp.com/api/appointment/${data.doctorId}`,
               { date: data.date },
               {
                  headers: {
  ...
  confirmFormMail: (info, user, doc, action) => {
    return async () => {
      try {
        let res = await axios.post("https://nutrimed.herokuapp.com/api/mail",{info, doc, action}, {
            headers: {
  ...
  getAvatars: () => {
   return async () => {
     try {
       let res = await axios.get("https://nutrimed.herokuapp.com/api/avatar");
       return { success: true, res: res.data.res };
     } catch (err) {
 ...
 getSocialWork:()=>{
   return async () => {
      try {
        let res = await axios.get("https://nutrimed.herokuapp.com/api/socialwork");
