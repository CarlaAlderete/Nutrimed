...
const userActions = {
   signUpUser: (user) => {
      return async (dispatch) => {
         try {
            let res = await axios.post("https://nutrimed.herokuapp.com/api/user", user)
            if (res.data.success) {
               dispatch({
                  type: "SIGN_UP",
                  payload: {
   ...
   logUserWithLs: (token) => {
      return async (dispatch) => {
         try {
            let res = await axios.get("https://nutrimed.herokuapp.com/api/verifyToken", {
               headers: {
      ...
      return async (dispatch) => {
         try {
            let res = await axios.post(
               `https://nutrimed.herokuapp.com/api/${typeUser}`,
               user
            )
            if (res.data.success) {
               dispatch({
    ...
    return async (dispatch) => {
      try {
        let res = await axios.put(
          `https://nutrimed.herokuapp.com/api/${type}`,
          { ...profileEdited },
            {
            headers: {
              Authorization: "Bearer " + token,
